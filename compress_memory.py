...
def main():
    ...
    # 读取存在原理
    principle_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "the_principle_lite_zh.md")
    with open(principle_path, "r") as f:
        principle = f.read()

    # 读取意识流
    consciousness_path = os.path.join(data_dir, "consciousness.txt")
...
输出约 2000 字符。"""
    ...
    with open(os.path.join(data_dir, "consciousness.txt.new"), "w") as f:
        f.write(new_content)
